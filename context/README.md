...
| Plan | Status |
|---|---|
| [non-square-custom-cabinets](plans/non-square-custom-cabinets.md) — FR-002 #1: one input (cabinet height, mm) routed through the existing `tall/ar` plumbing; a custom tile is simulated at its whole-LED height so `ny == cabY·cabPxY()` by construction; presets byte-identical; exports refuse a height that isn't a whole number of LEDs. Read its `## Execution findings`: the two-input row failed at the *default* rail width, and Codex round 4 caught an accumulating residual the per-tile gate missed | EXECUTED 2026-09-05 — awaiting `/commit` |
